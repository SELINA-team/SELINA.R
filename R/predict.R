...
#'
#' @param query_expr File path of the query data matrix(rds format).
...
#'
#' @examples
query_predict <- function(query_expr, model, path_meta, path_out, outprefix, disease = FALSE, mode = "single", seurat, cell_cutoff = 5, prob_cutoff = 0.9) {
  ...
  print("Loading data")
  query_expr <- readRDS(readRDS(query_expr)@assays$RNA@counts)
